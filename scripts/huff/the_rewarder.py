...
def main():
    ...
    flash_loan_pool = project.FlashLoanerPool.deploy(liquidity_token, sender=deployer)

    # set initial token balance of pool offering flash loans
    print("\n--- Funding flash loan pool with liquidity tokens ---\n")
    liquidity_token.transfer(flash_loan_pool, TOKENS_IN_LENDER_POOL, sender=deployer)

    # deploy rewarderPool, rewarderToken, and accountingToken
    print("\n--- Deploying reward pool, reward token, and accounting token ---\n")
    rewarder_pool = project.TheRewarderPool.deploy(liquidity_token, sender=deployer)
    ...
    assert accounting_token.owner() == rewarder_pool
    ...
    assert accounting_token.hasAllRoles(
        rewarder_pool, minter_role | snapshot_role | burner_role
    )
    ...
    for user in users:
        liquidity_token.transfer(user, deposit_amount, sender=deployer)
        liquidity_token.approve(rewarder_pool, deposit_amount, sender=user)
        rewarder_pool.deposit(deposit_amount, sender=user)
        assert accounting_token.balanceOf(user) == deposit_amount
    ...
    for user in users:
        ...
        assert reward_token.balanceOf(user) == rewards_in_round / len(users)
    ...
    assert liquidity_token.balanceOf(attacker) == 0

    # two rounds must have occured so far
    assert rewarder_pool.roundNumber() == 2

    # define intial balances for users and attackers
    alice_initial_bal = reward_token.balanceOf(users[0]) / 10**18
    bob_initial_bal = reward_token.balanceOf(users[1]) / 10**18
    lucy_initial_bal = reward_token.balanceOf(users[2]) / 10**18
    john_initial_bal = reward_token.balanceOf(users[3]) / 10**18
    attacker_initial_bal = reward_token.balanceOf(attacker) / 10**18
    ...
    for user in users:
        ...
        user_rewards = reward_token.balanceOf(user)
        delta = user_rewards - (rewarder_pool.REWARDS() / len(users))
        assert delta < 10**16

    # define ending balances for users and attacker
    alice_ending_bal = reward_token.balanceOf(users[0]) / 10**18
    bob_ending_bal = reward_token.balanceOf(users[1]) / 10**18
    lucy_ending_bal = reward_token.balanceOf(users[2]) / 10**18
    john_ending_bal = reward_token.balanceOf(users[3]) / 10**18
    attacker_ending_bal = reward_token.balanceOf(attacker) / 10**18
    ...
    attacker_rewards = reward_token.balanceOf(attacker)
    ...
    assert liquidity_token.balanceOf(attacker) == 0
    assert liquidity_token.balanceOf(flash_loan_pool) == TOKENS_IN_LENDER_POOL
