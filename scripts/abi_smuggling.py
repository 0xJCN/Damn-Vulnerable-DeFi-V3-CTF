...
def main():
    ...
    deployer_permission = vault.getActionId(
        "0x85fb709d", deployer.address, vault.address
    )
    attacker_permission = vault.getActionId(
        "0xd9caed12", attacker.address, vault.address
    )
    ...
    assert token.balanceOf(vault.address) == VAULT_TOKEN_BALANCE
    assert token.balanceOf(attacker.address) == 0

    # Cannot vall vault directly
    print("\n--- Ensuring you can not call the vault directly ---\n")
    with reverts():
        vault.sweepFunds(deployer.address, token.address, sender=deployer)
    with reverts():
        vault.withdraw(
            token.address,
            attacker.address,
    ...
    recovery_initial_bal = token.balanceOf(recovery.address) / 10**18
    ...
    send_tx(attacker.address, vault.address, malicious_calldata)
    ...
    recovery_ending_bal = token.balanceOf(recovery.address) / 10**18
    ...
    assert token.balanceOf(attacker.address) == 0
    assert token.balanceOf(recovery.address) == VAULT_TOKEN_BALANCE
