...
from .utils.helper import w3, reverts, ZERO_ADDRESS
...
def main():
    ...
    # using this extra account because vyper contracts
    # can not deploy contracts on their own
    accomplice = accounts.test_accounts[6]
    ...
    assert wallet_registry.owner() == deployer.address

    # Users are registered as beneficiaries
    print("\n--- Ensuring all users are registered as Beneficiaries ---\n")
    for user in users:
        # users are registered as beneficiaries
        assert wallet_registry.beneficiaries(user.address)
        # user cannot add beneficiary
        with reverts():
            wallet_registry.addBeneficiary(user.address, sender=user)

    # Transfer tokens to be distributed to the registry
    token.transfer(wallet_registry.address, AMOUNT_TOKEN_DISTRIBUTED, sender=deployer)

    # define initial balances for attacker and wallet
    wallet_initial_bal = token.balanceOf(wallet_registry.address) / 10**18
    ...
    # vyper can only create contracts from templates (contracts already deployed)
    # create_copy_of() is the equivalent of new or using the low level create opcode in solidity
    target = project.BackdoorHelper.deploy(sender=accomplice)

    project.BackdoorAttacker.deploy(
        target.address,
        wallet_factory.address,
        master_copy.address,
    ...
    wallet_ending_bal = token.balanceOf(wallet_registry.address) / 10**18
    ...
    for user in users:
        wallet = wallet_registry.wallets(user.address)

        # user must have a registered wallet
        assert wallet != ZERO_ADDRESS

        # user is no longer registered as a beneficiary
        assert not wallet_registry.beneficiaries(user.address)

    # attacker own all tokens
    assert token.balanceOf(attacker.address) == AMOUNT_TOKEN_DISTRIBUTED
