...
def main():
    ...
    set_balance(deployer.address, DEPLOYER_INITIAL_ETH_BALANCE)
    ...
    assert weth.balanceOf(deployer.address) == UNISWAP_INITIAL_WETH_LIQUIDITY
    ...
    weth.approve(
        uniswap_position_manager.address,
        MAX_UINT256,
        sender=deployer,
    )
    token.approve(
    ...
    #     domain_separator,
    # )
    # attacker_contract = project.PuppetV3Attacker.deploy(
    #     swap_router_address,
    #     lending_pool.address,
    #     token.address,
    #     weth.address,
    ...
    assert token.balanceOf(attacker.address) >= LENDING_POOL_INITIAL_TOKEN_BALANCE
