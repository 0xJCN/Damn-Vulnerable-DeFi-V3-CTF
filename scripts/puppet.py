...
def main():
    ...
    exchange_template = deploy_uniswap_contract("UniswapExchangeV1")

    # Deploy factory, initializing it with the address of the template exchange
    print("\n--- Deploying and initializing factory ---\n")
    uniswap_factory = deploy_uniswap_contract("UniswapFactoryV1")
    uniswap_factory.initializeFactory(exchange_template.address, sender=deployer)

    # Create a new exchange for the token and retrieve the deployed exchange's address
    print("\n--- Creating an exchange for our token ---\n")
    tx = uniswap_factory.createExchange(token.address, sender=deployer)
    ...
    token.approve(exchange.address, UNISWAP_INITIAL_TOKEN_RESERVE, sender=deployer)
    ...
    token.transfer(attacker.address, ATTACKER_INITAL_TOKEN_BALANCE, sender=deployer)
